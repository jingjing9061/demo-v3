<template>
    <div class="container">
           ...
           <template v-for="(item,idx) in listdata">
                <li v-if="!item.checked" :key="idx" @click="()=>item.checked = !item.checked">
                    <div >
                        ...
                        <label class="checklabel" :for="`item-${idx}`">{{item.name}}</label>
                    </div>
                </li>
           </template>
       </ul>
       <h3>已完成</h3>
        <ul>
           <li v-for="(item,idx) in finish" :key="idx" @click="()=>item.checked = !item.checked">
               <div>
                   <input type="checkbox" :id="`item-${idx}`" v-model="item.checked" disabled>
                   <label class="checklabel" :for="`item-${idx}`">{{item.name}}</label>
               </div>
           </li>
        </ul>
        ...
        <div>
            <label for="add"></label>
            <input id="add" type="text" placeholder="添加新的任务" v-model="value"/>
